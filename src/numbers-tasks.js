...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const upDrobie = x1 * x2 + y1 * y2;
  const leftDownDrobie = Math.sqrt(x1 * x1 + y1 * y1);
  const rightDownDrobie = Math.sqrt(x2 * x2 + y2 * y2);
  const cosinus = upDrobie / (leftDownDrobie * rightDownDrobie);
